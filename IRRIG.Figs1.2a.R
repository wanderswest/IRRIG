...
# open summarized data from Experiment 2 
nc<-open.ncdf("/Users/travis/GitHub/IRRIG/CESM.summary.data/difPRECC10.110Jul.nc") #(IRRIG-CTRL for July in simulation years 10-110)
...
PRECCJUL1<- ((P2$preccJ+P2$preccA)/2)*86400000*60
PT1<-P2$Pchg2
PL1<-P2$Pchg
P2$lon <- P2$lon-360
...
#################################
####### Experiment 3 RUNS (Yellowstone CISL)
...
PRECCJUL2<-((P3$IpreccJ+P3$IpreccA)-(P3$CpreccJ+P3$CpreccA))*86400000*30
PT2<- P3$Pchg2
PL2<- P3$Pchg
P3$lon2 <- P3$lon -360
...
#################################
#######  Experiment 1 mean preiciptiation type per month (irrig-ctrl)
...
# plot NOAA data mean precip total comparison transect
us_states <- map_data("state")
...
#Fig 1 PRECT data - Bring data together
...
#plot model range as polygon around mean (dashed line) 
